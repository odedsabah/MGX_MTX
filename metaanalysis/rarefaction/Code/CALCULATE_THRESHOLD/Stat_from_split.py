...
class MetaphlanAnalysis_from_split:
    ...
    def compute_jaccard_scores(self, df):
        ...
        for i in range(len(split_cols)):
            ...
            for j in range(i + 1, len(split_cols)):
                if (split_cols[i][1].split('.')[0] == split_cols[j][1].split('.')[0] and split_cols[i][0] != split_cols[j][0]):
        ...
        taxa_stats = pd.DataFrame(list(similarity_scores.items()), columns=['Cols_name', 'jaccard_scores']).set_index('Cols_name')
        return taxa_stats
    ...
    def sort_df(self, df, threshold):
        ...
        Taxa_stats.to_csv(f'~/out_stats_fastq/SAMEA110452918_out_stat/Taxa_stats_SAMEA110452918_split_{threshold}.csv')
